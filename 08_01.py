...
def get_row_halves(index, data): # leftmost is always the tree for comparison. simple max would fail by looking past tree
    row, col = index
...
def view_range(tree_input):
    ...
    other_trees = list(map(lambda n: 1 if n>= tree else 0, other_trees))
...
visible_count = 0
for i in range(len(forest)): # this could be double comprehension but we sleepy
